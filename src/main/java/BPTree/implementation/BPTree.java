...
public class BPTree<E extends Comparable<E>> {
    ...
    private LinkedList<CapsulateReault<E>> buffer;
    ...
    public BPTree(int leafSize, int internalSize, int bufferbool) {
        ...
        buffer = new LinkedList<CapsulateReault<E>>();
...
//======================================BULK INSERTION=================================================
    ...
    // =============================SEARCHING================================
    @SuppressWarnings("unchecked")
    public CapsulateReault<E> search(E key) {
        // secrhing in buffer array to check if the required
        // element on it or not
        for (int i = 0; i < buffer.size(); i++) {
            ArrayList<E> find = buffer.get(i).node.getKeys();
...
//            boolean found=false;
//            //sezrching fo the element
//            if (((BigInteger)key).and((BigInteger)n.getKeys().get(0)).xor((BigInteger)key).compareTo(BigInteger.ZERO)<0) {// if in the first pointer
//                n = (Node<E>) n.getPointers().get(0);
//            } else{
//                for (int i = 0; i < n.getKeys().size() - 1; i++) {
//                    if (n.getKeys().size() > 1 && (((BigInteger)key).and((BigInteger)n.getKeys().get(0)).xor((BigInteger)key).compareTo(BigInteger.ZERO)>=0) && (((BigInteger)key).and((BigInteger)n.getKeys().get(i+1)).xor((BigInteger)key).compareTo(BigInteger.ZERO)<0)) {// general case
//                        n = (Node) n.getPointers().get(i + 1);
//                        found = true;
//                        break;
//                    }
...
//        return null;
        do {
            for (int i = 0; i < n.getKeys().size(); i++) {
                BigInteger bkey=(BigInteger)key;
                BigInteger bkey2= (BigInteger) n.getKeys().get(i);
                if (bkey.testBit(47)==bkey2.testBit(47) && bkey.testBit(46)==bkey2.testBit(46)&& bkey.and(bkey2).xor(bkey).compareTo(BigInteger.ZERO) == 0) {
                   CapsulateReault<E> cpt= new CapsulateReault<E>(n , i , bkey2);
                    if (buffer.size() == bufferboolSize) {
                        buffer.removeFirst();
                        buffer.add(cpt);
                    } else {
    ...
    public Node<E> search2(E key) {
        ...
        for (int i = 0; i < n.getKeys().size(); i++) {
            if (key.compareTo(n.getKeys().get(i)) == 0) {
                if (buffer.size() == bufferboolSize) {
                    buffer.removeFirst();
                    buffer.add(new CapsulateReault<E>(n , i , (BigInteger) n.getKeys().get(i)));
                } else {
                    buffer.add(new CapsulateReault<E>(n , i , (BigInteger) n.getKeys().get(i)));
                }
                return n;
